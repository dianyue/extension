...
sogouExplorer.extension.onRequest.addListener(function(request, sender, sendResponse) {
    if(request.method == 'get_uid'){
        ...
        sendResponse(sogouExplorer.app.getDetails()[prop]);
        return true;
    }else {
...
function get_version(callback){
  callback(sogouExplorer.app.getDetails().version);
}

function get_platforms(callback){
  var platform = sogouExplorer.app.getDetails().platforms;
  if(!platform){
	  platform = 'mojing';
  }
  callback(platform);
...
function get_uid(callback){
	sogouExplorer.storage.local.get("uid", function(data){
	    var uid = data["uid"];
		if(!uid){
			uid = create_uid();
			sogouExplorer.storage.local.set({"uid": uid});
		}
        if(callback){
	        callback(uid);
		}
 ...
 */
sogouExplorer.runtime.onInstalled.addListener(function(){
	get_uid();
});
