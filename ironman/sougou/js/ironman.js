...
var store_get = function(key, callback) {
	sogouExplorer.storage.local.get(key, function(data){
	    if(callback){
		    callback(data[key]);
		}
	});
...
var store_set = function(key, value, callback) {
    var data = {};
    data[key] = value;
    sogouExplorer.storage.local.set(data);
    if(callback){
    	callback();
    }
};
...
var store_remove = function(key, callback) {
	sogouExplorer.storage.local.remove(key);
    if(callback){
    	callback();
    }
};
...
var get_extension_platform = function(callback) {
	prop_get("get_platforms", function(platform){
		if(callback){
    ...
    if(!window.ironman_user_term_sycm_btn){
    	...
    	dy_$(document.body).append('<div class="ironman-container">' +
            		'<div id="pop_user_term" class="alert center">' +
            			...
            			'<h4>尊敬的用户，请您阅读并接受<a href="http://ironman.moojing.com/terms_sycm_btn?uid='+ uid +'" target="_blank">'+ name +'用户协议</a>.如果不同意协议的话，将不能使用'+ name +'任何功能，请停止使用并卸载'+ name +'软件，谢谢！</h4>' +
